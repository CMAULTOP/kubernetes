...
|---|---|---:|---|---|---|---|
| `axum` | HTTP router, extraction и response composition API Server | `0.7.9` | MIT | Нужны типизированные routes и предсказуемые response/error boundaries без собственного HTTP stack. | Direct Hyper; Actix Web | Тонкий слой над Hyper, построен на Tower middleware и не навязывает data model. Pin `0.7.9` сохраняет стабильность первого API среза; workspace теперь собирается Rust `1.91`. [1] |
...
| `etcd-client` | Durable etcd v3 KV и transactional transport для ConfigMap repository | `0.16.0` | MIT OR Apache-2.0 | Нужны verified gRPC client, CAS transactions и server revisions; самодельный etcd client недопустим. | handwritten gRPC; raw Tonic generated etcd API | Отдельный `storage-etcd` crate использует client только для KV/Txn transport; Kubernetes mapping, validation и public error taxonomy остаются Rusternetes. [8] [9] |
...
Все перечисленные версии и лицензии фактических зависимостей получены из локально разрешённых Cargo manifests. Workspace minimum Rust version поднят до `1.91`, а integration proof persistence использует реальный etcd process. `http` и `idna_adapter` не считаются product dependencies: первый остаётся кандидатом на cleanup, второй остаётся транзитивной записью lockfile.
...
|---|---|---:|---|---|---|
| `etcd-client` | Async etcd v3 KV, transaction, watch, lease, lock, election и TLS client | `0.16.0` | Apache-2.0 OR MIT | Покрывает KV/Txn primitives; WatchClient остаётся низкоуровневым stream, поэтому recovery/ordering/compaction policy остаются ответственностью Rusternetes. [8] [9] | **Adopted.** `storage-etcd` implements ConfigMap key mapping, JSON payloads, etcd revision → opaque `resourceVersion`, CAS create/update/delete and real etcd integration proof. Durable HTTP WATCH remains a separate end-to-end slice. |
...
The existing ConfigMap code does not attempt to replace HTTP, JSON, time, UUID, base64, gRPC or client networking libraries. Its custom pieces are limited to Kubernetes server contract, validation, storage key mapping, resource-version ownership and public API errors. Before the controller slice, `kube-runtime` will be integrated instead of creating a parallel generic controller framework. Before the CRD slice, `kube-derive`/`schemars` will be used. Durable HTTP WATCH remains an explicit future integration of the selected etcd backend, not a fictitious claim of Phase 3.

The workspace now declares Rust `1.91` and the verified sandbox build uses Rust `1.91.1`; it exceeds the current MSRV stated by `etcd-client`. The client also needs `protoc` during its supported protobuf build step, which is part of the documented reproducible build environment. This removes the previous temporary Rust `1.75` restriction without substituting home-grown protocol implementations.
