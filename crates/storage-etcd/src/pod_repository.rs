//! Durable etcd v3 persistence for namespaced Pods.
//!
...
use rusternetes_api_types::{
    DeleteOptions, FieldSelector, LabelSelector, Pod, PodList, PodWatchEvent, PodWatchObject,
};
use rusternetes_common::{ApiError, ResourceReference};
use rusternetes_storage::{DeleteResult, PodWatchRequest};
...
const DEFAULT_PREFIX: &str = "/registry/pods";
const WATCH_CHANNEL_CAPACITY: usize = 256;

/// Durable repository mapping one Pod to one etcd key.
///
...
/// A bounded, cancellation-safe durable Pod WATCH subscription backed by etcd.
///
...
pub struct EtcdPodWatchSubscription {
    receiver: mpsc::Receiver<rusternetes_api_types::PodWatchEvent>,
    bridge_task: JoinHandle<()>,
}

impl EtcdPodWatchSubscription {
    pub async fn recv(&mut self) -> Option<rusternetes_api_types::PodWatchEvent> {
        self.receiver.recv().await
    }
}

impl Drop for EtcdPodWatchSubscription {
    fn drop(&mut self) {
        self.bridge_task.abort();
    }
}

impl EtcdPodRepository {
    ...
    /// Lists Pods from every namespace under this repository's dedicated etcd prefix.
    ...
    /// Opens a bounded Pod watch that replays etcd history and continues with live events.
    ///
    ...
    pub async fn watch(
        &self,
        request: PodWatchRequest,
    ) -> Result<EtcdPodWatchSubscription, ApiError> {
        ...
        let bridge_task = tokio::spawn(async move {
            loop {
                ...
                for event in response.events() {
                    ...
                    let PodWatchObject::Pod(resource) = &translated.object else {
                        continue;
                    };
    ...
    /// Replaces a Pod through an atomic mod-revision comparison.
    ///
    /// Kubernetes Pod permits an empty resourceVersion. The repository still reads the
    ...
    pub async fn update(&self, mut resource: Pod) -> Result<Pod, ApiError> {
        ...
        if stored.resource.metadata.deletion_timestamp.is_some() {
            resource.preserve_deletion_pending_update_from(&stored.resource, "0".to_owned());
        } else {
    ...
    async fn list_prefix(
        ...
        let items = response
            ...
            .collect();
        Ok(PodList::new(revision, items))
    }

    async fn get_stored(&self, namespace: &str, name: &str) -> Result<StoredPod, ApiError> {
        let reference = ResourceReference::pod(namespace.to_owned(), name.to_owned());
        let key = self.pod_key(namespace, name)?;
        let response = self
            .client
    ...
    fn pod_key(&self, namespace: &str, name: &str) -> Result<String, ApiError> {
...
fn watch_matches(request: &PodWatchRequest, resource: &Pod) -> bool {
    request
        .namespace
        .as_deref()
...
fn translate_watch_event(event: &etcd_client::Event) -> Result<Option<PodWatchEvent>, ApiError> {
    ...
    match event.event_type() {
        EtcdEventType::Put => {
            ...
            if key_value.version() == 1 {
                Ok(Some(PodWatchEvent::added(resource)))
            } else {
                Ok(Some(PodWatchEvent::modified(resource)))
            }
        }
        EtcdEventType::Delete => {
            ...
            Ok(Some(PodWatchEvent::deleted(resource)))
        }
    }
}
...
fn encode(resource: &Pod) -> Result<Vec<u8>, ApiError> {
    serde_json::to_vec(resource).map_err(|_| ApiError::Internal)
}

fn decode(raw: &[u8], mod_revision: i64) -> Result<Pod, ApiError> {
    let mut resource = serde_json::from_slice::<Pod>(raw).map_err(|_| ApiError::Internal)?;
    resource.enforce_type_meta()?;
...
mod tests {
    ...
    fn key_layout_rejects_ambiguous_components() {
        assert!(normalize_prefix("registry/pods").is_err());
        assert!(normalize_prefix("/registry/pods/").is_err());
