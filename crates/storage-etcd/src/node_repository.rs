//! Durable etcd v3 persistence for noded Nodes.
//!
...
use rusternetes_api_types::{
    DeleteOptions, FieldSelector, LabelSelector, Node, NodeList, NodeWatchEvent, NodeWatchObject,
};
use rusternetes_common::{ApiError, ResourceReference};
use rusternetes_storage::{DeleteResult, NodeWatchRequest};
...
const DEFAULT_PREFIX: &str = "/registry/nodes";
const WATCH_CHANNEL_CAPACITY: usize = 256;

/// Durable repository mapping one Node to one etcd key.
///
...
/// A bounded, cancellation-safe durable Node WATCH subscription backed by etcd.
///
...
pub struct EtcdNodeWatchSubscription {
    receiver: mpsc::Receiver<rusternetes_api_types::NodeWatchEvent>,
    bridge_task: JoinHandle<()>,
}

impl EtcdNodeWatchSubscription {
    pub async fn recv(&mut self) -> Option<rusternetes_api_types::NodeWatchEvent> {
        self.receiver.recv().await
    }
}

impl Drop for EtcdNodeWatchSubscription {
    fn drop(&mut self) {
        self.bridge_task.abort();
    }
}

impl EtcdNodeRepository {
    ...
    pub async fn create(&self, mut resource: Node) -> Result<Node, ApiError> {
        resource.enforce_type_meta()?;
        resource.validate_create()?;
        let name = resource.name()?.to_owned();
        let reference = ResourceReference::node(name.clone());
        let key = self.node_key(&name)?;
    ...
    /// Opens a bounded Node watch that replays etcd history and continues with live events.
    ///
    ...
    ) -> Result<EtcdNodeWatchSubscription, ApiError> {
        ...
        let bridge_task = tokio::spawn(async move {
            loop {
                ...
                for event in response.events() {
                    ...
                    let NodeWatchObject::Node(resource) = &translated.object else {
                        continue;
                    };
    ...
    /// Replaces a Node through an atomic mod-revision comparison.
    ///
    /// Kubernetes Node permits an empty resourceVersion. The repository still reads the
    /// current object and compares its observed etcd revision, so the write never becomes blind.
    pub async fn update(&self, mut resource: Node) -> Result<Node, ApiError> {
        resource.enforce_type_meta()?;
        let name = resource.name()?.to_owned();
        let reference = ResourceReference::node(name.clone());
    ...
    /// Replaces only the status projection of a Node through an atomic etcd mod-revision compare.
    pub async fn update_status(&self, mut resource: Node) -> Result<Node, ApiError> {
        resource.enforce_type_meta()?;
        let name = resource.name()?.to_owned();
        let reference = ResourceReference::node(name.clone());
        let requested_version = resource.metadata.resource_version.clone();
    ...
    pub async fn delete(
        ...
        let reference = ResourceReference::node(name);
        let stored = self.get_stored(name).await?;
        ...
        let items = response
            ...
            .filter(|resource| {
                label_selector.matches(&resource.metadata.labels)
                    && field_selector.matches_node(resource)
            })
            .collect();
        Ok(NodeList::new(revision, items))
    }

    async fn get_stored(&self, name: &str) -> Result<StoredNode, ApiError> {
        let reference = ResourceReference::node(name);
        let key = self.node_key(name)?;
        let response = self
            .client
    ...
    fn node_key(&self, name: &str) -> Result<String, ApiError> {
...
fn watch_matches(request: &NodeWatchRequest, resource: &Node) -> bool {
    request.label_selector.matches(&resource.metadata.labels)
        && request.field_selector.matches_node(resource)
}

fn translate_watch_event(event: &etcd_client::Event) -> Result<Option<NodeWatchEvent>, ApiError> {
    ...
    match event.event_type() {
        EtcdEventType::Put => {
            ...
            if key_value.version() == 1 {
                Ok(Some(NodeWatchEvent::added(resource)))
            } else {
                Ok(Some(NodeWatchEvent::modified(resource)))
            }
        }
        EtcdEventType::Delete => {
            ...
            Ok(Some(NodeWatchEvent::deleted(resource)))
        }
    }
}
...
fn encode(resource: &Node) -> Result<Vec<u8>, ApiError> {
    serde_json::to_vec(resource).map_err(|_| ApiError::Internal)
}

fn decode(raw: &[u8], mod_revision: i64) -> Result<Node, ApiError> {
    let mut resource = serde_json::from_slice::<Node>(raw).map_err(|_| ApiError::Internal)?;
    resource.enforce_type_meta()?;
...
mod tests {
    ...
    fn key_layout_rejects_ambiguous_components() {
        assert!(normalize_prefix("registry/nodes").is_err());
        assert!(normalize_prefix("/registry/nodes/").is_err());
        assert!(validate_key_component("node", "default/other").is_err());
